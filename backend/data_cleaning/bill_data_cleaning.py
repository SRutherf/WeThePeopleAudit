...
def connect_bill_people_vote(bills_dir, output_dir=None):
    """
    Connect bill, people, and vote JSON data by establishing relationships.
    Creates a connected dataset that links all three types of data.
    Saves output to each year's folder, as well as a central output directory.
    ...
    # Define central output directory if not specified
    ...
    # Initialize data structures to hold consolidated data
    connected_data = {
        "years": {},
        "all_bills": {},
        "all_people": {},
    ...
    for year_dir in year_dirs:
            ...
            for bill_file in bill_files:
                ...
                if bill_json and "bill" in bill_json:
                    ...
                    if bill_id:
                        ...
                        # Extract sponsors (relationship to people)
        ...
        # Process vote files (if they exist)
            ...
            for vote_file in vote_files:
                ...
                if vote_json and "roll_call" in vote_json:
                    ...
                    if roll_call_id:
                        ...
                        # Connect individual votes to people
    ...
    # Save full connected data (may be very large)
    save_json_file(connected_data, output_dir / "full_connected_data.json")

    # Save individual relationship files (more manageable)
    ...
    # Save year-by-year data to both central output directory and original year folders
    for year, year_data in connected_data["years"].items():
        # Save to central output directory
...
def create_database_format(connected_data, output_dir, bills_dir):
    """
    Creates CSV files in database-friendly format for loading into a relational database.
    Saves both to central output directory and to each year folder.
    ...
    # Create CSV files for main entities
    bills_rows = []
    ...
    # Convert to CSV for central output
    import pandas as pd
    ...
    # Also save year-specific CSVs to each year folder
    for year, year_data in connected_data["years"].items():
...
def main():
    ...
    if connected_data:
        # Create database-friendly format (both in central location and year folders)
