...
#df = pd.read_csv('combined_spending.csv', low_memory=False)

# Complete dictionary to map cities to their districts
district_mapping = {
    # District 1
...
# Function to assign the district based on city and state
def assign_district(row):
    # If the state is 'Unassigned'
    if row['state'] == 'Unassigned':
        return 'Unknown'

    # If the city is in the mapping and state is 'Massachusetts'
    if row['state'] == 'MA' and row['city'] in district_mapping:
        return district_mapping.get(row['city'], 'Unknown')

    # If it's from a different state
    return 'Other state'
...
df['Congressional District'] = df.apply(assign_district, axis=1)
df['Senate District'] = df.apply(assign_senate_district, axis=1)
df['House District'] = df.apply(assign_house_district, axis=1)

df.to_csv('updated_spending_with_districts.csv', index=False)
